...
ifeq ($(OS),Windows_NT)
	ifeq (true, $(APP_DEBUG))
		FLAT_COMPILER_PATH := $(realpath $(APP_PROJECT_PATH)/../build-windows/bin/$(APP_ABI)/Debug/flatc.exe)
	else
		FLAT_COMPILER_PATH := $(realpath $(APP_PROJECT_PATH)/../build-windows/bin/$(APP_ABI)/Release/flatc.exe)
	endif
else
	UNAME := $(strip $(shell uname -s 2>/dev/null))

	ifeq ($(UNAME),Darwin)
		FLAT_COMPILER_PATH := $(realpath $(APP_PRODUCT_PATH)/flatc)
	else
		ifeq (true, $(APP_DEBUG))
			FLAT_COMPILER_PATH := $(realpath $(APP_PROJECT_PATH)/../build-linux/bin/debug/$(APP_ABI)/flatc)
		else
			FLAT_COMPILER_PATH := $(realpath $(APP_PROJECT_PATH)/../build-linux/bin/release/$(APP_ABI)/flatc)
		endif
	endif
endif
