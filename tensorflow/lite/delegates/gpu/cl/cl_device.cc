...
#error Unknown Compiler
            ...
            {
              ABSL_RAW_LOG(WARNING, "clGetDeviceInfo returned %d", static_cast<int>(status_get_device_host_unified_memory));
            }
          }
        }
        else
        {
          ABSL_RAW_LOG(ERROR, "clGetDeviceIDs returned %d", static_cast<int>(status_get_device_id_2));
        }
      }
      else
      {
        ABSL_RAW_LOG(WARNING, "No GPU on %d platform.", static_cast<int>(reinterpret_cast<intptr_t>(platform_id)));
      }
    }
    else
    {
      ABSL_RAW_LOG(ERROR, "clGetDeviceIDs returned %d", static_cast<int>(status_get_device_id_1));
    }
  }
